...
uint32_t support_pse; // Whether the system supports variable page sizes
...
// Checks whether processor supports variable page size
static void
...
static void boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm, int use_pse);
...
mem_init(void)
{
	...
	// Find out whether the machine supports variable page size
	check_pse_availability();
	...
	boot_map_region(kern_pgdir, UPAGES, PTSIZE, PADDR(pages), PTE_U | PTE_P, 0);
	...
	boot_map_region(kern_pgdir, UENVS, PTSIZE, PADDR(envs), PTE_U | PTE_P, 0);
	...
	boot_map_region(kern_pgdir, KSTACKTOP-KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_P|PTE_W, 0);
	...
	boot_map_region(kern_pgdir, KERNBASE, (2^32)-KERNBASE, 0, PTE_P|PTE_W, 0);

	// Set up SMP-related regions of memory
	mem_init_mp();
...
{	
	// Notes on 4MB pages:
	// - The use_pse bit is located on bit 1 of alloc_flags.
	//   I will change this behavior if it clashes with future features.
	// - Since for now, the only time we will allocate 4MB pages is in
	//   boot_map_region() in mem_init(), when no pages have been freed
	//   and page_free_list is still contiguous, we will allocate 1024
	//   pages. we will implement a check to assure contiguous-ness.
	// - Note that coalescing the pages are not necessary as long as we
	//   make sure that these pages are contiguous in physical memory
	//   and that they are always accesses 4MB at a time.

	if (page_free_list == NULL)
		return NULL;
	
	// if (alloc_flags & 0x2) {	// Use 4MB page
	// 	struct PageInfo *allocated_pages[1024];
	...
	// 		allocated_pages[i] = allocated_page;
	// 	}

	// 	for (int i = 0; i < 1023; i++) {	// Check for page contiguous-ness
	// 		if (page2pa(allocated_pages[i]) - page2pa(allocated_pages[i+1]) != PGSIZE)
	// 			panic("4KB pages allocated not contiguous in page_alloc()");
	// 	}

	// 	if (alloc_flags & ALLOC_ZERO)
	// 		memset(page2kva(allocated_page), 0, LPGSIZE);

	// 	return allocated_pages[1023];
...
//	the page is cleared,
//	and pgdir_walk returns a pointer into the new page table page(entry?).
//
...
static void
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm, int use_pse)
{
	// Fill this function in
	// TODO
	...
	if (use_pse && support_pse){	// Use 4MB pages
		...
		for (size_t i = 0; i < size / LPGSIZE; i++) {
			...
			cprintf("pde_addr: %p\n", pde_addr);
			if (pde_addr == NULL)
				panic("pgdir_walk() returns NULL when mapping a 4MB page in boot_map_region()\n");
		...
		if (use_pse)
			cprintf("boot_map_region wants to use 4MB pages but 4MB pages not supported by system, falling back to 4KB pages\n");
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pte_t *pte_addr = pgdir_walk(pgdir, va, 1, 0);
	if (pte_addr == NULL)
		return -E_NO_MEM;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	pte_t *pte_addr = pgdir_walk(pgdir, va, 0, 0);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t ptr_va = (uintptr_t)va;
	uintptr_t start_va = ROUNDDOWN(ptr_va, PGSIZE);
    uintptr_t end_va = ROUNDUP(ptr_va+len, PGSIZE);
	uintptr_t curr;
	int pgnum = (end_va-start_va)/PGSIZE;

	for (int i = 0; i < pgnum; i++) {
		curr = start_va + i*PGSIZE;
		pte_t *pte = pgdir_walk(env->env_pgdir, (void*)curr, 0, 0);
		if (curr >= ULIM || pte == NULL || !(*pte & PTE_P) || (*pte & perm) != perm) {
			user_mem_check_addr = (curr < ptr_va) ? ptr_va : curr;
			return -E_FAULT;
		}
	}
...
check_kern_pgdir(void)
{
	...
	for (i = 0; i < n; i += PGSIZE) {
		// cprintf("Linear address of page %d: %x\n", i, UPAGES + i);
		// cprintf("Physical address corresponding to this linear address: %x\n", check_va2pa(pgdir, UPAGES + i));
		// cprintf("Physical address of page %d: %x\n", i, PADDR(pages) + i);
	...
	// check phys mem - 4KB version
	for (i = 0; i < npages * PGSIZE; i += PGSIZE) {
		// cprintf("i: %x, va: %p, pa: %p\n", i, KERNBASE + i, check_va2pa(pgdir, KERNBASE + i));
		assert(check_va2pa(pgdir, KERNBASE + i) == i);
	}
	// check phys mem - 4MB version
	// for (i = 0; i < npages * PGSIZE; i += LPGSIZE) {
		// cprintf("va of current 4MB page: %x\n", KERNBASE+i);
		// cprintf("pa from va of current 4MB page: %x\n", check_va2pa(pgdir, KERNBASE + i));
		// cprintf("pa of current 4MB page: %x\n", i);
		// assert(check_va2pa(pgdir, KERNBASE + i) == i);
	// }
...
check_page(void)
{
	...
	assert(pgdir_walk(kern_pgdir, (void*)PGSIZE, 0, 0) == ptep+PTX(PGSIZE));

	// should be able to change permissions too.
	assert(page_insert(kern_pgdir, pp2, (void*) PGSIZE, PTE_W|PTE_U) == 0);
	assert(check_va2pa(kern_pgdir, PGSIZE) == page2pa(pp2));
	assert(pp2->pp_ref == 1);
	assert(*pgdir_walk(kern_pgdir, (void*) PGSIZE, 0, 0) & PTE_U);
	assert(kern_pgdir[0] & PTE_U);

	// should be able to remap with fewer permissions
	assert(page_insert(kern_pgdir, pp2, (void*) PGSIZE, PTE_W) == 0);
	assert(*pgdir_walk(kern_pgdir, (void*) PGSIZE, 0, 0) & PTE_W);
	assert(!(*pgdir_walk(kern_pgdir, (void*) PGSIZE, 0, 0) & PTE_U));
	...
	ptep = pgdir_walk(kern_pgdir, va, 1, 0);
	...
	// check permissions
	assert(*pgdir_walk(kern_pgdir, (void*) mm1, 0, 0) & (PTE_W|PTE_PWT|PTE_PCD));
	assert(!(*pgdir_walk(kern_pgdir, (void*) mm1, 0, 0) & PTE_U));
	// clear the mappings
	*pgdir_walk(kern_pgdir, (void*) mm1, 0, 0) = 0;
	*pgdir_walk(kern_pgdir, (void*) mm1 + PGSIZE, 0, 0) = 0;
	*pgdir_walk(kern_pgdir, (void*) mm2, 0, 0) = 0;
