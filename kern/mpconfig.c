...
struct mp {             // floating pointer [MP 4.1]
	uint8_t signature[4];           // ASCII string "_MP_", for searching
...
struct mpconf {         // configuration table header [MP 4.2]
	uint8_t signature[4];           // String "PCMP"
...
// Also implements checksum checking
static struct mp *
...
// 1) in the first KB of the EBDA (which lives at the top of low mem);
