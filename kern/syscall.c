...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// Check perm - potential bug with ~PTE_SYSCALL
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// cprintf("Check 2 passed\n");

	// Check for error 4
	if (!((perm & PTE_U) && (perm & PTE_P) && !(perm & (~PTE_SYSCALL))))
		return -E_INVAL;

	// cprintf("Check 4 passed\n");
...
// Note that we don't implement perm checking. Be nice!
static int
sys_env_set_priority(envid_t envid, uint8_t priority)
{
	// cprintf("In kernel sys_env_set_priority, envid: 0x%x\n", envid);
	struct Env *env;
	int r;
